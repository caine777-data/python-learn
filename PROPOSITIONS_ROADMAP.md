...
| N° | Intitulé du parcours | Thème & Modules standard utilisés | Nombre de leçons |
|:---:|---|---|:---:|
| **17** | **Cybersécurité & Cryptographie** | Hachage, temps constant, tokens aléatoires, prévention SQLi/Path Traversal (`hashlib`, `hmac`, `secrets`, `pathlib`) | 6 leçons + 1 quiz |
| **18** | **Mathématiques, Sciences & Simulations** | Nombres premiers, précision exacte, Monte-Carlo, Newton, vecteurs (`math`, `cmath`, `decimal`, `fractions`, `random`) | 6 leçons + 1 quiz |
| **19** | **Traitement d'Images & Audio** | Pixels RGB, formats PPM/WAV en pur Python, filtres, synthèse de sons (`wave`, `struct`, `io`) | 6 leçons + 1 quiz |
| **20** | **Intelligence Artificielle (de zéro)** | k-NN, régression linéaire, perceptron, analyse de sentiments, recommandation cosinus | 6 leçons + 1 quiz |
| **21** | **Réseaux & Protocoles** | Sockets TCP/UDP, client/serveur, échange JSON, résolution DNS, mini-chat (`socket`, `threading`, `select`) | 6 leçons + 1 quiz |
| **22** | **Architecture & Jeux Vidéo 2D** | Game loop, grille 2D, collisions, inventaire, tour par tour, IA ennemie, sauvegarde JSON | 6 leçons + 1 quiz |
| **23** | **Design Patterns & Typage Moderne** | Singleton, Factory, Observateur, Stratégie, Protocoles, Enums, ABC (`typing`, `abc`, `enum`) | 6 leçons + 1 quiz |

---

## ⚡ II. Nouvelles Fonctionnalités Logicielles Proposées
...
- **Mode « Défi du Jour » (Daily Challenge)** : sélectionne automatiquement un exercice ou un quiz quotidien (basé sur le jour calendaire) pour entretenir sa série de jours (`streak 🔥`).
...
- **Toile Turtle intégrée directement sur Canvas Tkinter** : affichage du tracé en direct pour le parcours Turtle sans ouvrir de fenêtre séparée.
...
- **Explorateur visuel de tables SQLite** : grille affichant en direct les tables de la base de données en mémoire et leurs modifications lors des requêtes.
...
- **Exportateur de paquets Anki (`.tsv`)** : réviser le glossaire et les concepts sur smartphone.
...
- **Inspecteur de Bytecode CPython (`dis`)** : voir les instructions machine virtuelles (`LOAD_FAST`, `BINARY_OP`, etc.).
